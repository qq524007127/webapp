...
<head>
    ...
    <script type="text/javascript">
        ...
        //自定义福位架编号
        function editSelfCode() {
            var checkeds = $('.box-content p input[type=checkbox]:checked');
            if (checkeds.length != 1) {
                $.messager.alert('', '请选择一个需要自定义编号的福位架');
                return;
            }
            var shelf = {};
            shelf.shelfId = $(checkeds[0]).val();
            shelf.shelfCode = $('#' + shelf.shelfId).html();
                    ...
                    handler: function () {
                        $('#editCodeForm').form('submit',{
                            success: function (data) {
                                ...
                                if (data.success) {
                                    $('#' + shelf.shelfId).html($('#editCodeForm input[name=shelfCode]').val());
                                ...
                                <div class="box enable">
                                    ...
                                    <div class="box disable">
                                        ...
                                        <div class="box-content">
                                            ...
                                            <p>编号：<span id="${shelf.shelfId}">${shelf.shelfCode}</span></p>
...
<div id="toolbarPanel">
    ...
    <a class="easyui-linkbutton" iconCls="icon-edit" onclick="editSelfCode()">自定义编号</a>
    <a class="easyui-linkbutton" iconCls="icon-reload" onClick="location.reload()">刷新</a>
</div>

<div id="editCodeDialog">
    <form id="editCodeForm" method="post" action="${pageContext.request.contextPath}/api/shelfPlan_editCode.action">
        <input name="shelfId" type="hidden">
        <table class="form-container" align="center">
            <tr>
                <td class="title"><label>自定义编号：</label></td>
                <td>
                    <p>
                        <input name="shelfCode" class="easyui-validatebox" data-options="required:true">
                    </p>
                </td>
            </tr>
        </table>
    ...
    <form id="addForm" method="post" action="${pageContext.request.contextPath }/api/shelfPlan_add.action">
        ...
        <table class="form-container" align="center">
            ...
            <tr>
                ...
                <td>
                    <input name="areaRow" type="text" class="easyui-numberbox" value="${area.areaRow + 1 }"
                           data-options="min:1,max:${area.areaRow + 1 },precision:0,required:true">
                </td>
            </tr>
            <tr>
                <td class="title"><label>所在区域列：</label></td>
                <td>
                    <input name="areaColumn" type="text" class="easyui-numberbox" value="${area.areaColumn + 1 }"
                           data-options="min:1,max:${area.areaColumn + 1 },precision:0,required:true">
                </td>
            </tr>
        </table>
    </form>
